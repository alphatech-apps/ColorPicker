...
public class FlowerColorWheelRenderer extends AbsColorWheelRenderer {
	...
	@Override
	public void draw() {
		final int setSize = presetsColorCircleList.size();
		int currentCount = 0;
		...
		for (int i = 0; i < density; i++) {
			...
			for (int j = 0; j < total; j++) {
				...
				if (currentCount >= setSize) {
					presetsColorCircleList.add(new PresetsColorCircle(x, y, hsv));
				} else presetsColorCircleList.get(currentCount).set(x, y, hsv);
				currentCount++;
			}
		}
	}
